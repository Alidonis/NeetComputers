...
public class GraphicalAPI implements Exposable
{
    ...
    @Exposed
    public void setColor(@Index( strict = true, offset=-1 ) @Range( range = 256 ) int R, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int G, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int B)//sets the color used when not specificly provided by a draw function
    {
        defualtColor = new BadVector(R,G,B);
    }
    @Exposed
    public void setColor(@Index( strict = true, offset=-1 ) @Range( range = 256 ) int R, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int G, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int B, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int A)//sets the color used when not specificly provided by a draw function
    {
        setColor(R,G,B);
        alpha = A;
    }
    ...
    @Exposed
    public void drawLine(@Index int x0, @Index int y0, int x1, int y1, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int R, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int G, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int B) {
    ...
    public void drawLayer(@Index int x, @Index int y, Table layer){
        ...
        if (rasterizedUUID.get()==null) throw new ExposedError("Table provided not valid layer1");
    ...
    public Table copy(@Index int x1, @Index int y1, int x2, int y2){
        ...
        if (sizex==0 || sizey==0) throw new ExposedError("Copy range has a dimension of 0 size");
    ...
    @Exposed
    public void floodFill(int x, int y, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int tolerance, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int R, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int G, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int B){
        if (x < 0 || x >= width) throw new ExposedError("Argument #0: value not in range [0-"+width+']');
        if (y < 0 || y >= height) throw new ExposedError("Argument #1: value not in range [0-"+height+']');
        if (tolerance==255){
            fill(R, G, B);
            return;
        }
    ...
    @Exposed
    public void fill(@Index( strict = true, offset=-1 ) @Range( range = 256 ) int R,@Index( strict = true, offset=-1 ) @Range( range = 256 ) int G,@Index( strict = true, offset=-1 ) @Range( range = 256 ) int B){
    ...
    @Exposed
    public void fill(@Index int x1, @Index int y1,  int x2,  int y2, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int R,@Index( strict = true, offset=-1 ) @Range( range = 256 ) int G,@Index( strict = true, offset=-1 ) @Range( range = 256 ) int B){
    ...
    @Exposed
    public void drawRectangle(@Index int x1, @Index int y1, int x2, int y2, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int R, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int G, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int B)//draws a rectangle between two points
    {
    ...
    public void drawBezier(BadVector pointA, BadVector pointB, BadVector pointC, BadVector color)//draw a bezier curver through the provided points
    {
    ...
    @Exposed
    public void drawBezier(@Index int x1, @Index int y1, @Index int x2, @Index int y2, @Index int x3, @Index int y3, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int R, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int G, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int B){
    ...
    public void drawSpline(List Xs, List Ys){
        if (!Xs.check((val) -> val.instanceOf(VarType.NUMBER))) throw new ExposedError("Argument #1: all vals in list must be numbers");
        if (!Ys.check((val) -> val.instanceOf(VarType.NUMBER))) throw new ExposedError("Argument #2: all vals in list must be numbers");
        if (Xs.size()!= Ys.size()) throw new ExposedError("drawSpline called with balance of X and Y values");
        drawSpline(Xs.cast((val) -> val.toInt()).toArray(new Integer[]{}),Ys.cast((val) -> val.toInt()).toArray(new Integer[]{}), defualtColor);
    }
    @Exposed
    public void drawSpline(List Xs, List Ys, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int R, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int G, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int B){
        if (!Xs.check((val) -> val.instanceOf(VarType.NUMBER))) throw new ExposedError("Argument #1: all vals in list must be numbers");
        if (!Ys.check((val) -> val.instanceOf(VarType.NUMBER))) throw new ExposedError("Argument #2: all vals in list must be numbers");
        if (Xs.size()!= Ys.size()) throw new ExposedError("drawSpline called with balance of X and Y values");
        drawSpline(Xs.cast((val) -> val.toInt()).toArray(new Integer[]{}),Ys.cast((val) -> val.toInt()).toArray(new Integer[]{}),new BadVector(R,G,B));
    }
    ...
    public void drawSpline(List cords){
        if (!cords.check((val) -> val.instanceOf(VarType.NUMBER))) throw new ExposedError("Argument #1: all vals in list must be numbers");
        if (cords.size()%2==1) throw new ExposedError("Argument #1: list input contained a uneven amount of values (not valid list of coordinates)");
        drawSpline(cords.cast((val) -> val.toInt()).toArray(new Integer[]{}),defualtColor);
    }
    @Exposed
    public void drawSpline(List cords, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int R, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int G, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int B){
        if (!cords.check((val) -> val.instanceOf(VarType.NUMBER))) throw new ExposedError("Argument #1: all vals in list must be numbers");
        if (cords.size()%2==1) throw new ExposedError("Argument #1: list input contained a uneven amount of values (not valid list of coordinates)");
    ...
    public void drawCircle(BadVector pointA, BadVector pointB, BadVector color)//draw a circle inbetween the two points
    {
        ...
        BadVector point = new BadVector((pointA.x+pointB.x)/2,(pointA.x+pointB.x)/2);
    ...
    @Exposed
    public void drawCircle(@Index int x1, @Index int y1, int x2, int y2, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int R, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int G, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int B){
    ...
    public void drawCircle(BadVector point, int radius, BadVector color)//draw a circle of given radius around point
    {
    ...
    @Exposed
    public void drawCircle(@Index int x, @Index int y, int radius, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int R, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int G, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int B){
    ...
    @Exposed
    public void drawPolygon(@Index int x, @Index int y, int n, int size, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int R, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int G, @Index( strict = true, offset=-1 ) @Range( range = 256 ) int B){
